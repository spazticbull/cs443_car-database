...
<body>
    <?php
    // variables
    $email = $firstName = $lastName = $streetAddr = $cityAddr = $stateAddr = $zipAddr = $phone = $empType = "";
    ...
    <div class="container">
        ...
        <div class="row">
            <form method="post" class="col s12" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" name="addEmp">
                <div class="row">
                    ...
                    <div class="col s12 radioSection">
                        <div class="row">
                            <h6>What position do you currently hold?</h6>
                        </div>
                        <div class="row">
                            <input type="radio" class="with-gap" name="empType" id="custServ" value="Customer Service"/>
                            <label for="custServ">Customer Service</label>
                        </div>
                        <div class="row">
                            <input type="radio" class="with-gap" name="empType" id="service" value="Service"/>
                            <label for="service">Service</label>
                        </div>
                        <div class="row">
                            <input type="radio" class="with-gap" name="empType" id="sales" value="Sales"/>
                            <label for="sales">Sales</label>
                        </div>
                        <div class="row">
                            <input type="radio" class="with-gap" name="empType" id="admin" value="Administrative"/>
                            <label for="admin">Administrative</label>
                        </div>
                        <div class="row">
            ...
            <?php
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    if (isset($_REQUEST["action"])) {
                        $sql = "INSERT INTO employee (firstname, lastname, email, street, city, state, zip, phone, type) VALUES 
                        (' $firstName ', ' $lastName ', ' $email ', ' $streetAddr ', ' $cityAddr ', ' $stateAddr ', ' $zipAddr ', ' $phone ', ' $empType ');";
                    
                        if ($conn->query($sql) === TRUE) {
                            echo "Employee " . $firstName . " " . $lastName . " has been added to the database successfully";
        ...
        $(document).ready(function() {
            $('#addEmp').validate({
                ...
                errorPlacement: function(error, element) {
                    ...
                    console.log(element);
                    if (element.is(":radio")) {
                        $("#radioError").text("Please choose current position");
                    } else if (placement) {
